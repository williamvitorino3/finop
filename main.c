...
int padrao(struct Clientes *clientes, struct Contas *contas, struct Operacoes *operacoes, struct Transacoes *transacoes, struct Transacoes_cartao_credito *transacoes_cartao)
{
  while (1)
  {
    switch (getOpcao())
...
int main(int argc, char *argv[ ])
{
  ...
  if (argc > 1)
  {
    ...
    if (!strcmp(argv[1], "clientes_estado"))
    {
      if (argc-2 == 1)
      {
        filtroEstado(clientes, contas, argv[2]);
      }
    }else
  ...
  return padrao(clientes, contas, operacoes, transacoes, transacoes_cartao);
}
